...
def _settle_from_results_locked(
    ...
    for idx in df[open_mask].index:
        ...
        if market == "home":
            ...
            won = 2 <= (hg + ag) <= 4
        elif market in ("h1_goals_2_4", "h2_goals_2_4", "h1_goals_2_4_no", "h2_goals_2_4_no"):
            # Half-time scores are not available in the final-score feed.
            # Keep the bet open for manual settlement instead of voiding it.
            continue
