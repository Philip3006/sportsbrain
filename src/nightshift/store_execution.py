"""Leasing, completion, recovery, and query operations for the dispatcher store."""

from __future__ import annotations

import json
from datetime import datetime, timedelta
from typing import Any

from .errors import InvalidTransitionError, LeaseError
from .models import (
    EventType,
    ExecutionResult,
    TaskRecord,
    TaskState,
    isoformat,
    parse_timestamp,
    utc_now,
)
from .store_schema import ALLOWED_TRANSITIONS


class StoreExecutionMixin:
    """Atomic queue operations used by worker processes."""

    def claim_next(
        self,
        builder_id: str,
        *,
        worker_id: str,
        lease_seconds: int,
        max_concurrency: int = 1,
        now: datetime | None = None,
    ) -> TaskRecord | None:
        current = now or utc_now()
        timestamp = isoformat(current)
        expiry = isoformat(current + timedelta(seconds=lease_seconds))
        with self._write() as conn:
            leased_count = conn.execute(
                "SELECT COUNT(*) FROM tasks WHERE builder_id = ? AND state = 'leased'",
                (builder_id,),
            ).fetchone()[0]
            if leased_count >= max_concurrency:
                return None
            rows = conn.execute(
                """SELECT * FROM tasks
                   WHERE builder_id = ? AND state IN ('queued', 'retry_wait') AND available_at <= ?
                   ORDER BY priority DESC, created_at ASC, task_id ASC""",
                (builder_id, timestamp),
            ).fetchall()
            active_locks = [
                set(json.loads(item[0]))
                for item in conn.execute(
                    "SELECT resource_locks_json FROM tasks WHERE state = 'leased' AND resource_locks_json != '[]'"
                ).fetchall()
            ]
            for row in rows:
                candidate_locks = set(json.loads(row["resource_locks_json"]))
                if candidate_locks and any(
                    candidate_locks & held for held in active_locks
                ):
                    self._append_event(
                        conn,
                        row["task_id"],
                        EventType.RESOURCE_LOCK_WAIT,
                        worker_id,
                        timestamp,
                        row["state"],
                        row["state"],
                        {"resource_locks": sorted(candidate_locks)},
                    )
                    continue
                dependencies = tuple(json.loads(row["dependency_ids_json"]))
                if dependencies:
                    dep_rows = conn.execute(
                        f"SELECT task_id, state FROM tasks WHERE task_id IN ({','.join('?' for _ in dependencies)})",
                        dependencies,
                    ).fetchall()
                    dep_states = {
                        dep["task_id"]: TaskState(dep["state"]) for dep in dep_rows
                    }
                    if any(dep_id not in dep_states for dep_id in dependencies):
                        continue
                    if any(
                        dep_states[dep_id]
                        in {
                            TaskState.FAILED,
                            TaskState.FAILED_SAFE,
                            TaskState.DEAD_LETTER,
                            TaskState.CANCELLED,
                            TaskState.BLOCKED,
                        }
                        for dep_id in dependencies
                    ):
                        conn.execute(
                            "UPDATE tasks SET state = 'blocked', updated_at = ?, last_error = ? WHERE task_id = ?",
                            (timestamp, "dependency did not succeed", row["task_id"]),
                        )
                        self._append_event(
                            conn,
                            row["task_id"],
                            EventType.BLOCKED,
                            worker_id,
                            timestamp,
                            row["state"],
                            TaskState.BLOCKED.value,
                            {"dependencies": list(dependencies)},
                        )
                        continue
                    if any(
                        dep_states[dep_id]
                        not in {
                            TaskState.SUCCEEDED,
                            TaskState.PR_READY,
                            TaskState.CEO_REVIEW,
                        }
                        for dep_id in dependencies
                    ):
                        continue
                new_attempt = row["attempt_count"] + 1
                conn.execute(
                    """UPDATE tasks SET state = 'leased', attempt_count = ?, updated_at = ?,
                       lease_owner = ?, lease_expires_at = ?, last_error = NULL, failure_class = NULL
                       WHERE task_id = ? AND state IN ('queued', 'retry_wait')""",
                    (new_attempt, timestamp, worker_id, expiry, row["task_id"]),
                )
                active_locks.append(candidate_locks)
                self._append_event(
                    conn,
                    row["task_id"],
                    EventType.CLAIMED,
                    worker_id,
                    timestamp,
                    row["state"],
                    TaskState.LEASED.value,
                    {
                        "attempt": new_attempt,
                        "lease_expires_at": expiry,
                        "resource_locks": sorted(candidate_locks),
                    },
                )
                return self._record(self._get_row(conn, row["task_id"]))
        return None

    def heartbeat(
        self,
        task_id: str,
        *,
        worker_id: str,
        lease_seconds: int,
        now: datetime | None = None,
    ) -> TaskRecord:
        current = now or utc_now()
        timestamp = isoformat(current)
        expiry = isoformat(current + timedelta(seconds=lease_seconds))
        with self._write() as conn:
            row = self._get_row(conn, task_id)
            if (
                row["state"] != TaskState.LEASED.value
                or row["lease_owner"] != worker_id
            ):
                raise LeaseError("heartbeat requires the active lease owner")
            if (
                row["lease_expires_at"]
                and parse_timestamp(row["lease_expires_at"]) <= current
            ):
                raise LeaseError("cannot heartbeat an expired lease")
            conn.execute(
                "UPDATE tasks SET updated_at = ?, lease_expires_at = ? WHERE task_id = ?",
                (timestamp, expiry, task_id),
            )
            self._append_event(
                conn,
                task_id,
                EventType.HEARTBEAT,
                worker_id,
                timestamp,
                TaskState.LEASED.value,
                TaskState.LEASED.value,
                {"lease_expires_at": expiry},
            )
            return self._record(self._get_row(conn, task_id))

    def complete(
        self,
        task_id: str,
        *,
        worker_id: str,
        execution: ExecutionResult,
        retry_at: datetime | None = None,
        now: datetime | None = None,
    ) -> TaskRecord:
        current = now or utc_now()
        timestamp = isoformat(current)
        with self._write() as conn:
            row = self._get_row(conn, task_id)
            if (
                row["state"] != TaskState.LEASED.value
                or row["lease_owner"] != worker_id
            ):
                raise LeaseError("completion requires the active lease owner")
            if (
                row["lease_expires_at"]
                and parse_timestamp(row["lease_expires_at"]) <= current
            ):
                raise LeaseError("completion arrived after the lease expired")
            previous = TaskState.LEASED
            result_json = self._json(execution.as_dict())
            if execution.success:
                new_state = execution.terminal_state or TaskState.SUCCEEDED
                event = (
                    EventType.PR_READY
                    if new_state is TaskState.PR_READY
                    else EventType.CEO_REVIEW
                    if new_state is TaskState.CEO_REVIEW
                    else EventType.SUCCEEDED
                )
                details = {"summary": execution.summary}
                updates = {
                    "lease_owner": None,
                    "lease_expires_at": None,
                    "result_json": result_json,
                    "last_error": None,
                    "failure_class": None,
                }
            elif execution.retryable and row["attempt_count"] < row["max_attempts"]:
                new_state, event = TaskState.RETRY_WAIT, EventType.RETRY_SCHEDULED
                next_at = isoformat(retry_at or current)
                details = {
                    "summary": execution.summary,
                    "attempt": row["attempt_count"],
                    "available_at": next_at,
                }
                updates = {
                    "lease_owner": None,
                    "lease_expires_at": None,
                    "result_json": result_json,
                    "last_error": execution.summary or "worker failed",
                    "available_at": next_at,
                    "failure_class": None,
                }
            elif execution.retryable:
                new_state, event = TaskState.DEAD_LETTER, EventType.DEAD_LETTERED
                details = {
                    "summary": execution.summary,
                    "attempt": row["attempt_count"],
                    "reason": "attempt budget exhausted",
                }
                updates = {
                    "lease_owner": None,
                    "lease_expires_at": None,
                    "result_json": result_json,
                    "last_error": execution.summary or "attempt budget exhausted",
                    "failure_class": "DEAD_LETTER",
                }
            else:
                new_state, event = TaskState.FAILED, EventType.FAILED
                details = {
                    "summary": execution.summary,
                    "attempt": row["attempt_count"],
                    "retryable": False,
                }
                new_state = (
                    TaskState.FAILED_SAFE
                    if execution.terminal_state is TaskState.FAILED_SAFE
                    else new_state
                )
                event = (
                    EventType.FAILED_SAFE
                    if new_state is TaskState.FAILED_SAFE
                    else event
                )
                updates = {
                    "lease_owner": None,
                    "lease_expires_at": None,
                    "result_json": result_json,
                    "last_error": execution.summary or "non-retryable worker failure",
                    "failure_class": "FAILED_SAFE"
                    if new_state is TaskState.FAILED_SAFE
                    else None,
                }
            if new_state not in ALLOWED_TRANSITIONS[previous]:
                raise InvalidTransitionError(
                    f"{previous.value} -> {new_state.value} is not permitted"
                )
            assignments = ["state = ?", "updated_at = ?"]
            values: list[Any] = [new_state.value, timestamp]
            for column, value in updates.items():
                assignments.append(f"{column} = ?")
                values.append(value)
            values.append(task_id)
            conn.execute(
                f"UPDATE tasks SET {', '.join(assignments)} WHERE task_id = ?", values
            )
            self._append_event(
                conn,
                task_id,
                event,
                worker_id,
                timestamp,
                previous.value,
                new_state.value,
                details,
            )
            return self._record(self._get_row(conn, task_id))

    def recover_expired(
        self, *, now: datetime | None = None, actor: str = "reaper"
    ) -> list[TaskRecord]:
        current = now or utc_now()
        timestamp = isoformat(current)
        recovered: list[TaskRecord] = []
        with self._write() as conn:
            rows = conn.execute(
                "SELECT * FROM tasks WHERE state = 'leased' AND lease_expires_at IS NOT NULL AND lease_expires_at <= ? ORDER BY task_id",
                (timestamp,),
            ).fetchall()
            for row in rows:
                if row["attempt_count"] < row["max_attempts"]:
                    new_state = TaskState.RETRY_WAIT
                    event = EventType.LEASE_EXPIRED
                    available = timestamp
                else:
                    new_state = TaskState.DEAD_LETTER
                    event = EventType.DEAD_LETTERED
                    available = row["available_at"]
                conn.execute(
                    """UPDATE tasks SET state = ?, updated_at = ?, available_at = ?,
                       lease_owner = NULL, lease_expires_at = NULL, last_error = ?, failure_class = ? WHERE task_id = ?""",
                    (
                        new_state.value,
                        timestamp,
                        available,
                        "lease expired before completion",
                        "STALE_WORKER",
                        row["task_id"],
                    ),
                )
                self._append_event(
                    conn,
                    row["task_id"],
                    event,
                    actor,
                    timestamp,
                    TaskState.LEASED.value,
                    new_state.value,
                    {
                        "previous_owner": row["lease_owner"],
                        "attempt": row["attempt_count"],
                    },
                )
                recovered.append(self._record(self._get_row(conn, row["task_id"])))
        return recovered

    def cancel(
        self, task_id: str, *, actor: str, reason: str = "", now: datetime | None = None
    ) -> TaskRecord:
        current = now or utc_now()
        record = self.get(task_id)
        if record.state is TaskState.LEASED:
            raise LeaseError("leased work must finish or expire before cancellation")
        return self.transition(
            task_id,
            expected=record.state,
            new_state=TaskState.CANCELLED,
            actor=actor,
            event_type=EventType.CANCELLED,
            details={"reason": reason},
            now=current,
            updates={"last_error": reason or "cancelled"},
        )

    def list_tasks(
        self,
        *,
        state: TaskState | None = None,
        builder_id: str | None = None,
        limit: int = 100,
    ) -> list[TaskRecord]:
        if not 1 <= limit <= 1000:
            raise ValueError("limit must be between 1 and 1000")
        clauses: list[str] = []
        params: list[Any] = []
        if state is not None:
            clauses.append("state = ?")
            params.append(state.value)
        if builder_id is not None:
            clauses.append("builder_id = ?")
            params.append(builder_id)
        where = f"WHERE {' AND '.join(clauses)}" if clauses else ""
        with self._read() as conn:
            rows = conn.execute(
                f"SELECT * FROM tasks {where} ORDER BY created_at DESC, task_id DESC LIMIT ?",
                (*params, limit),
            ).fetchall()
            return [self._record(row) for row in rows]
