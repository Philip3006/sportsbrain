...
def _add_issue(
    ...
    field: str,
    detail: str,
    invalid_fixture_keys: set[str] | None = None,
) -> None:
    key = IntegrityCode(code).value
    counts[key] += 1
    if invalid_fixture_keys is not None and source != "results" and fixture_key:
        invalid_fixture_keys.add(fixture_key)
    candidate = IntegrityIssue(code, source, fixture_key, field, detail)
    if len(issues) < limit:
        issues.append(candidate)
        return
    # Retain the lexicographically smallest samples rather than the first
    # samples encountered.  This keeps the bounded diagnostic invariant while
    # making the payload independent of source-row order.
    sort_key = lambda issue: (
        issue.code.value,
        issue.source,
        issue.fixture_key,
        issue.field,
    ...
    worst_index, worst = max(enumerate(issues), key=lambda item: sort_key(item[1]))
    if sort_key(candidate) < sort_key(worst):
        issues[worst_index] = candidate
...
def audit_champions_league_inputs(
    ...
    *,
    ...
    invalid_fixture_keys: set[str] = set()
    add = lambda *args: _add_issue(
        issues,
        counts,
        policy.max_issue_samples,
        *args,
        invalid_fixture_keys=invalid_fixture_keys,
    )
    inventory = {
    ...
    for raw in results:
        ...
        if timestamp is not None:
            try:
                ...
                inventory["timestamp_inputs"] += 1
                if (
                    result_at < expected[3]
                    and _as_text(_row_value(raw, "status")).casefold()
    ...
    for raw in odds:
        ...
        if captured_at > expected[3] and captured_at > expected[3].replace(microsecond=0):
            if (captured_at - expected[3]).total_seconds() > policy.post_kickoff_tolerance_seconds:
                add(IntegrityCode.POST_KICKOFF_CONTAMINATION, "odds", key, "captured_at", "odds were captured after kickoff")
    ...
    return ChampionsLeagueIntegrityReport(
        ...
        invalid_fixture_keys=frozenset(
            key for key in invalid_fixture_keys if key in fixture_map
        ),
    )
