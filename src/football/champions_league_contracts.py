...
class ChampionsLeagueIntegrityReport:
    ...
    # Kept for coverage accounting, but intentionally omitted from the public
    # payload so a large replay cannot expand an observability record with one
    # entry per invalid fixture.
    invalid_fixture_keys: frozenset[str] = frozenset()
    ...
    def raise_if_invalid(self) -> None:
        if self.issue_counts:
            ...
            if first is None:
                raise ChampionsLeagueIntegrityError("integrity report is invalid")
            raise ChampionsLeagueIntegrityError(
