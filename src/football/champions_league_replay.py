...
def build_champions_league_replay_coverage(
    ...
    *,
    ...
    def source_fixture_key(row: object, source: str) -> str:
        if not isinstance(row, Mapping):
            return ""
        normalized = _nested_row(row) if source == "serializer" else row
        return _fixture_key(normalized)

    attached = {
        source: {
            source_fixture_key(row, source)
            for row in rows
            if source_fixture_key(row, source) in fixture_keys
        }
        for source, rows in (
            ("odds", odds),
            ("features", features),
    ...
    result_keys = {
        source_fixture_key(row, "results")
        for row in results
        if source_fixture_key(row, "results") in fixture_keys
    }
    resolved_keys = {
        source_fixture_key(row, "results")
        for row in results
    ...
    issue_fixture_keys = report.invalid_fixture_keys
    legitimate = sum(
