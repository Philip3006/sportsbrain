...
### J2-H. + NEU Surface-aware TENNIS_CATEGORY_MODE
- **Was**: Aktueller `TENNIS_CATEGORY_MODE` ist nur per Kategorie. J2-G-Backtest zeigt: Surface-Edge dominiert — atp500 grass +18.6% vs hard -8.8%, wta250 grass +16% vs hard -2%. Einführung `TENNIS_CATEGORY_SURFACE_MODE: dict[(category, surface), str]` damit profitable Kombinationen freigeschaltet werden ohne pauschale Kategorie-Risiken.
- **Warum**: Aktuell blockiert: 6 LIVE-Kombinationen aus J2-G-Backtest fallen unter `shadow`-Default, weil ihre Kategorie negative Gesamt-ROI hat.
- **Impact/Aufwand/Risiko**: 🟢 (mehr Live-Edges) · 🟡 (3-4 h: Datenstruktur + Detector-Lookup + Tests) · 🟡 (kleinere Stichproben pro Subkategorie — n≥50 Gate weiterhin gilt)
- **Priorität**: P2 (lohnt sich erst wenn Tennis-Saison läuft; aktuell off-season)
- **Dateien**: `src/config.py` (neue Map), `scripts/tennis_scan.py::category_mode`, `tests/scanner/test_tennis_scan.py`
- **Verifikation**: Scanner-Smoke mit gemockten Mode-Lookups, Backtest-Verdict-Tabelle als Test-Fixture
...
| **12d** | J2-D2 Follow-up (WebSearch-Fallback-Adaption für 2-way Tennis) | wenn nötig nach erstem Datenpunkt | 2-3 h |
...
- **P1**: 26 — davon 24 ✅ (J2 Phase A-F erledigt 2026-06-23/24; offen: M5 blockiert bis 2026-06-27, J2-D2 nur wenn nötig)
- **P2**: 14 — davon 8 ✅ (E1, E2, E3, E4, H1, H2, I6, I7); offen: I1–I5, J3, J4
- **P3**: 4 (Q4 2026)
